...
jest.mock('@/components/pages/projects/project-view', () => ({
	__esModule: true,
	default: (props: { session?: Session; id?: number }) => {
		...
		return React.createElement(
			'div',
			null,
			`PROJECT_VIEW_CLIENT_MARKER:${JSON.stringify(props?.session ?? null)}:ID=${props?.id ?? ''}`,
		);
	},
}));
...
describe('ProjectDetailPage server component', () => {
	...
	it('renders ProjectViewClient with session and id when session exists', async () => {
		const sessionValue: Session = { user: { pk: 10, email: 'viewer@projects.com' } };
	...
	it('redirects to PROJECTS_LIST when id is invalid', async () => {
		const sessionValue: Session = { user: { pk: 10, email: 'viewer@projects.com' } };
		mockAuth.mockResolvedValueOnce(sessionValue);
