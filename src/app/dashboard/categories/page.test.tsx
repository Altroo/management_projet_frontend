...
jest.mock('@/components/pages/categories/categories-list', () => ({
	__esModule: true,
	default: (props: { session?: Session }) => {
		// eslint-disable-next-line @typescript-eslint/no-require-imports
		const React = require('react');
		return React.createElement('div', null, `CATEGORIES_LIST_CLIENT_MARKER:${JSON.stringify(props?.session ?? null)}`);
	},
}));
...
describe('CategoriesListPage server component', () => {
	...
	it('renders CategoriesListClient with session when session exists', async () => {
		const sessionValue: Session = { user: { pk: 4, email: 'admin@categories.com' } };
		...
		expect(decoded).toContain('"email":"admin@categories.com"');
