...
describe('RevenueFormClient', () => {
	beforeEach(() => {
		jest.clearAllMocks();
		mockUseGetRevenueQuery.mockReturnValue({
			data: undefined,
			isLoading: false,
			error: undefined,
		});
	...
	describe('Add Mode (no id)', () => {
		it('renders form fields', () => {
			renderWithProviders(<RevenueFormClient session={mockSession} />);
		...
		it('renders submit button with add text', () => {
			renderWithProviders(<RevenueFormClient session={mockSession} />);
			expect(screen.getByTestId('submit-button')).toHaveTextContent('Nouveau revenu');
		});
	...
	describe('Edit Mode (with id)', () => {
		it('renders submit button with update text', () => {
			...
			renderWithProviders(<RevenueFormClient session={mockSession} id={10} />);
			expect(screen.getByTestId('submit-button')).toHaveTextContent('Mettre à jour');
		});
	});

	describe('Hook calls', () => {
		it('calls useGetRevenueQuery when in edit mode', () => {
			renderWithProviders(<RevenueFormClient session={mockSession} id={789} />);
			expect(mockUseGetRevenueQuery).toHaveBeenCalledWith({ id: 789 }, expect.any(Object));
		});
	});
});
